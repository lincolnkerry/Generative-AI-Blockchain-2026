...
사용자 프롬프트
      ...
      ▼
┌─────────────┐
│  Extractor  │  ← SLM이 맥락적으로 민감 정보 탐지 + load-bearing 판정
│  (SLM)      │
└──────┬──────┘
┌─────────────┐
│   Router    │  ← rule-based 정책 결정 (is_load_bearing 기반)
│ (Rule-based)│
└──────┬──────┘
       │
       ├─ allow       → 민감 정보 없음, 외부 API로 직접 전송
...
SLM을 사용하여 맥락적으로 민감 정보를 탐지합니다. **Three-harm test** 프레임워크를 적용합니다:
...
| `GET/POST/DELETE` | `/api/v1/keys` | ✅ | API 키 관리 (SHA-256 해시 저장) |
| `GET/POST/DELETE` | `/api/v1/models` | ✅ | 모델 등록/조회/삭제 |
...
| 도구 | 파라미터 | 반환 | 설명 |
|---|---|---|---|
| `classify` | `text: str` | `{is_sensitive, records, policy_action, requires_masking, description}` | 프롬프트 분류 |
| `route` | `text: str` | (동일) | `classify`의 별칭 |
...
```
┌─────────────────┐     OTLP gRPC      ┌──────────────────┐
│  Privacy Router  │ ──────────────────▶ │  OTel Collector   │
│  (FastAPI)       │  :4317             │  :4317 / :4318    │
│                  │                    └────────┬─────────┘
│  OTel SDK        │                             │
│  + FastAPI inst. │              ┌──────────────┼──────────────┐
│                  │              ▼              ▼              ▼
...
```
 ...
 server/api/routes (FastAPI endpoints)
        │
        ▼
 agents/router (PrivacyRouter)
        │
   ┌────┼────┐
   ▼    ▼    ▼
extractor  masker  llm.py
   │              │
   ▼              ▼
config/      litellm + instructor
(YAML config, model registry)
```
...
- [docs/INTEGRATION_ARCHITECTURE.md](docs/INTEGRATION_ARCHITECTURE.md) — 연동 아키텍처 상세
