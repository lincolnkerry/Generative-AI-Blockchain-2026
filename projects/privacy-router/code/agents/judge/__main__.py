...
def main():
    print("=" * 70)
    print("Privacy Router - Judge Demo")
    print("=" * 70)
    print()
    ...
    for i, sample in enumerate(SAMPLE_INPUTS, 1):
        ...
        judgment = judge.classify(records_dict, sample["text"])

        print("[판정 결과]")
        print(f"  마스킹 필요: {'Yes' if judgment.needs_masking else 'No'}")
        print(f"  마스킹 후 유의미: {'Yes' if judgment.meaningful_after_masking else 'No'}")
        print(f"  추천 엔드포인트: {judgment.recommended_endpoint}")
        print()
